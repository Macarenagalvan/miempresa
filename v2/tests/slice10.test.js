...
import { ingestPrint, applyResolution, parseRgmJsonl, DEFAULT_RGM_SOURCE_ASSET } from "../js/adapters/rgm.js";
import { RGM_SLICE10_LINES, RGM_SLICE10_SOURCE_ASSET, asJsonl } from "../js/fixtures/rgm-slice10.js";
import { RGM_SOURCE_ASSET } from "../js/config.js";
...
async function run() {
  ...
  const mapped = ingestPrint(RGM_SLICE10_LINES.resolvedTp, { sourceAsset: asset });
  ...
  const createdTp = await ingestRgmPayload(RGM_SLICE10_LINES.resolvedTp, stage.id, { sourceAsset: asset });
  ...
  const createdSl = await ingestRgmPayload(RGM_SLICE10_LINES.resolvedSl, stage.id, { sourceAsset: asset });
  assert("SL → resolution SL", createdSl.signal.resolution === Resolution.SL && createdSl.signal.disposition === Disposition.NONE);

  const createdSkip = await ingestRgmPayload(RGM_SLICE10_LINES.skippedOpen, stage.id, { sourceAsset: asset });
  ...
  const createdOpen = await ingestRgmPayload(RGM_SLICE10_LINES.openedLong, stage.id, { sourceAsset: asset });
  const createdShort = await ingestRgmPayload(RGM_SLICE10_LINES.openedShort, stage.id, { sourceAsset: asset });
  ...
  const again = await ingestRgmPayload(RGM_SLICE10_LINES.resolvedTp, stage.id, { sourceAsset: asset });
  ...
  const updated = await ingestRgmPayload(RGM_SLICE10_LINES.laterTpSameId, stage.id, { sourceAsset: asset });
  ...
  const conflict = await ingestRgmPayload(RGM_SLICE10_LINES.conflictPrint, stage.id, { sourceAsset: asset });
  assert("conflicto no sobrescribe print", conflict.status === "conflict" && (await findSignalByRgmId("rgm-real-tp")).direction === "LONG");

  const unknown = await ingestRgmPayload(RGM_SLICE10_LINES.unknownResolved, stage.id, { sourceAsset: asset });
  ...
  const badId = await ingestRgmPayload(RGM_SLICE10_LINES.invalidNoId, stage.id, { sourceAsset: asset });
  const badSide = await ingestRgmPayload(RGM_SLICE10_LINES.invalidNoSide, stage.id, { sourceAsset: asset });
  const badAlert = await ingestRgmPayload(RGM_SLICE10_LINES.invalidNoAlert, stage.id, { sourceAsset: asset });
  assert("payload inválido no corrompe", badId.status === "invalid" && badSide.status === "invalid" && badAlert.status === "invalid");

  try {
    await syncRgmJsonl(asJsonl([RGM_SLICE10_LINES.openedLong]), stage.id, { sourceAsset: asset });
  ...
  const sync1 = await syncRgmJsonl(
    ...
    stage.id,
    { sourceAsset: asset, syncFrom: "2026-08-27T00:00:00.000Z" },
  );
  ...
  const afterManual = await ingestRgmPayload(RGM_SLICE10_LINES.resolvedTp, stage.id, { sourceAsset: asset });
