...
export async function renderNumeros(ctx) {
  ...
  [asset, strategy, variant, direction, session, from, to].forEach((n) => n.addEventListener("change", apply));
  return [
    el("section", { className: "panel" }, [
      el("p", { className: "kicker", text: "Universo Backtest · stage activa" }),
      el("h1", { text: "Números" }),
      el("p", { className: "meta", text: "Real / Demo / Desk no disponibles. No se mezclan." }),
      el("div", { className: "chips filters" }, [asset, strategy, variant, direction, session, from, to]),
